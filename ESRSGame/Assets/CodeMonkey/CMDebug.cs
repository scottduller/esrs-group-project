...
namespace CodeMonkey {
    ...
    public static class CMDebug {
        ...
        public static void Text(string text, Vector3 localPosition = default(Vector3), Transform parent = null, int fontSize = 40, Color? color = null, TextAnchor textAnchor = TextAnchor.UpperLeft, TextAlignment textAlignment = TextAlignment.Left, int sortingOrder = CmUtilsClass.sortingOrderDefault) {
            CmUtilsClass.CreateWorldText(text, parent, localPosition, fontSize, color, textAnchor, textAlignment, sortingOrder);
        }
        ...
        public static void TextPopupMouse(string text, Vector3? offset = null) {
            ...
            CmUtilsClass.CreateWorldTextPopup(text, UtilsClass.GetMouseWorldPosition() + (Vector3)offset);
        }
        ...
        public static void TextPopup(string text, Vector3 position, float popupTime = 1f) {
            CmUtilsClass.CreateWorldTextPopup(text, position, popupTime);
        }

        // Text Updater in World, (parent == null) = world position
        public static FunctionUpdater TextUpdater(Func<string> GetTextFunc, Vector3 localPosition, Transform parent = null, int fontSize = 40, Color? color = null, TextAnchor textAnchor = TextAnchor.UpperLeft, TextAlignment textAlignment = TextAlignment.Left, int sortingOrder = CmUtilsClass.sortingOrderDefault) {
            return CmUtilsClass.CreateWorldTextUpdater(GetTextFunc, localPosition, parent, fontSize, color, textAnchor, textAlignment, sortingOrder);
        }

        // Text Updater in UI
        public static FunctionUpdater TextUpdaterUI(Func<string> GetTextFunc, Vector2 anchoredPosition) {
            return CmUtilsClass.CreateUITextUpdater(GetTextFunc, anchoredPosition);
        }
        ...
        public static FunctionUpdater KeyCodeAction(KeyCode keyCode, Action onKeyDown) {
            return CmUtilsClass.CreateKeyCodeAction(keyCode, onKeyDown);
        }
