    ...
    public static class CmUtilsClass {
        ...
        public static FunctionUpdater CreateMouseDraggingAction(int mouseButton, Action<Vector3> onMouseDragging) {
            ...
            return FunctionUpdater.Create(() => {
                ...
                if (dragging) {
                    onMouseDragging(CmUtilsClass.GetMouseWorldPosition());
                }
                return false; 
            });
        }
        ...
        public static FunctionUpdater CreateMouseClickFromToAction(int mouseButton, int cancelMouseButton, Action<Vector3, Vector3> onMouseClickFromTo, Action<Vector3, Vector3> onWaitingForToPosition) {
            ...
            return FunctionUpdater.Create(() => {
                if (state == 1) {
                    if (onWaitingForToPosition != null) onWaitingForToPosition(from, CmUtilsClass.GetMouseWorldPosition());
                }
                if (state == 1 && Input.GetMouseButtonDown(cancelMouseButton)) {
                    // Cancel
                    state = 0;
                }
                if (Input.GetMouseButtonDown(mouseButton) && !CmUtilsClass.IsPointerOverUI()) {
                    if (state == 0) {
                        state = 1;
                        from = CmUtilsClass.GetMouseWorldPosition();
                    } else {
                        state = 0;
                        onMouseClickFromTo(from, CmUtilsClass.GetMouseWorldPosition());
                    }
                }
                return false; 
            });
        ...
        public static bool IsPositionInsideFov(Vector3 pos, Vector3 aimDir, Vector3 posTarget, float fov) {
            int aimAngle = CmUtilsClass.GetAngleFromVector180(aimDir);
            int angle = CmUtilsClass.GetAngleFromVector180(posTarget - pos);
        ...
        public static List<Vector3> GetPositionListAround(Vector3 position, float distance, int positionCount) {
            ...
            for (int i = 0; i < positionCount; i++) {
                ...
                Vector3 dir = CmUtilsClass.ApplyRotationToVector(new Vector3(0, 1), angle);
        ...
        public static List<Vector3> GetPositionListAround(Vector3 position, float distance, int positionCount, Vector3 direction, int angleStart, int angleIncrease) {
            ...
            for (int i = 0; i < positionCount; i++) {
                ...
                Vector3 dir = CmUtilsClass.ApplyRotationToVector(direction, angle);
                Vector3 pos = position + dir * distance;
                ret.Add(pos);
            }
            return ret;
        ...
        public static List<Vector2Int> GetPosXYListOblong(int width, int dropXamount, int increaseDropXamount, Vector3 dir) {
            ...
            list = RotatePosXYList(list, CmUtilsClass.GetAngleFromVector(dir));
            return list;
        }
        ...
        public static List<Vector2Int> RotatePosXYList(List<Vector2Int> list, int angle) {
            ...
            for (int i = 0; i < list.Count; i++) {
                ...
                Vector3 vec = CmUtilsClass.ApplyRotationToVector(new Vector3(posXY.x, posXY.y), angle);
        ...
        public static void DebugDrawCircle(Vector3 center, float radius, Color color, float duration, int divisions) {
            for (int i = 0; i <= divisions; i++) {
                Vector3 vec1 = center + CmUtilsClass.ApplyRotationToVector(new Vector3(0, 1) * radius, (360f / divisions) * i);
                Vector3 vec2 = center + CmUtilsClass.ApplyRotationToVector(new Vector3(0, 1) * radius, (360f / divisions) * (i + 1));
