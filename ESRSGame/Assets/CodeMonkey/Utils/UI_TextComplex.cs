...
namespace CodeMonkey.Utils {
    ...
    public class UI_TextComplex {
        ...
        private static Transform GetCanvasTransform() {
            return CmUtilsClass.GetCanvasTransform();
        }
        ...
        public UI_TextComplex(Transform parent, Vector2 anchoredPosition, int fontSize, char iconChar, string text, Icon[] iconArr, Font font) {
            ...
            while (tmp.IndexOf(iconChar) != -1) {
                ...
                if (untilTmp.Trim() != "") {
                    Text uiText = CmUtilsClass.DrawTextUI(untilTmp, transform, new Vector2(textPosition,0), fontSize, font);
                    textPosition += uiText.preferredWidth;
                }
                // Draw Icon
                int iconIndex = CmUtilsClass.Parse_Int(iconNumber, 0);
                Icon icon = iconArr[iconIndex];
                CmUtilsClass.DrawSprite(icon.sprite, transform, new Vector2(textPosition + icon.size.x / 2f, 0), icon.size);
                textPosition += icon.size.x;
            }
            if (tmp.Trim() != "") {
                CmUtilsClass.DrawTextUI(tmp, transform, new Vector2(textPosition,0), fontSize, font);
            }
        }
