...
namespace CodeMonkey.Utils {
    ...
    public class UI_Bar {
        ...
        private RectTransform SetupOutline(Outline outline, Vector2 size) {
            return CmUtilsClass.DrawSprite(outline.color, gameObject.transform, Vector2.zero, size + new Vector2(outline.size, outline.size), "Outline");
        }

        private void SetupBackground(Color backgroundColor) {
            background = CmUtilsClass.DrawSprite(backgroundColor, gameObject.transform, Vector2.zero, Vector2.zero, "Background");
            background.anchorMin = new Vector2(0,0);
            background.anchorMax = new Vector2(1,1);
        }

        private void SetupBar(Color barColor) {
            bar = CmUtilsClass.DrawSprite(barColor, gameObject.transform, Vector2.zero, Vector2.zero, "Bar");
